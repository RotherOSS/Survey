...
sub RequestSend {
    ...
    if ( $ConfigObject->Get('Survey::CheckSendConditionCustomerFields') ) {
        ...
        if ( IsHashRefWithData( $Survey{CustomerUserConditions} ) ) {
            ...
            return if !eval($Results);    ## no critic qw(BuiltinFunctions::ProhibitStringyEval)
        }
    }

    for my $Data ( sort keys %Ticket ) {
        if ( defined $Ticket{$Data} ) {
            $Subject =~ s/<OTOBO_TICKET_$Data>/$Ticket{$Data}/gi;
            $Body    =~ s/<OTOBO_TICKET_$Data>/$Ticket{$Data}/gi;

            # filter for new rich text content
            $Body =~ s/&lt;OTOBO_TICKET_$Data&gt;/$Ticket{$Data}/g;
        }
    }

    # cleanup
    $Subject =~ s/<OTOBO_TICKET_.+?>/-/gi;
    $Body    =~ s/<OTOBO_TICKET_.+?>/-/gi;

    # replace config options
    $Subject =~ s{<OTOBO_CONFIG_(.+?)>}{$ConfigObject->Get($1)}egx;
    $Body    =~ s{<OTOBO_CONFIG_(.+?)>}{$ConfigObject->Get($1)}egx;

    # filter for new rich text content
    $Body =~ s{&lt;OTOBO_CONFIG_(.+?)&gt;}{$ConfigObject->Get($1)}egx;

    # cleanup
    $Subject =~ s/<OTOBO_CONFIG_.+?>/-/gi;
    $Body    =~ s/<OTOBO_CONFIG_.+?>/-/gi;
    ...
    if (%CustomerUser) {
        ...
        for my $Data ( sort keys %CustomerUser ) {
            ...
            $Subject =~ s/<OTOBO_CUSTOMER_DATA_$Data>/$CustomerUser{$Data}/gi;
            $Body    =~ s/<OTOBO_CUSTOMER_DATA_$Data>/$CustomerUser{$Data}/gi;
    ...
    $Subject =~ s/<OTOBO_CUSTOMER_DATA_.+?>/-/gi;
    $Body    =~ s/<OTOBO_CUSTOMER_DATA_.+?>/-/gi;

    # filter for new rich text content
    $Body =~ s/&lt;OTOBO_CUSTOMER_DATA_.+?&gt;/-/gi;

    # replace key
    $Subject =~ s/<OTOBO_PublicSurveyKey>/$PublicSurveyKey/gi;
    $Body    =~ s/<OTOBO_PublicSurveyKey>/$PublicSurveyKey/gi;
