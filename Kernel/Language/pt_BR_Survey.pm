...
sub Data {
    ...
    $Self->{Translation}->{'Please choose a Customer property to add a condition.'}
        = 'Por favor, escolha uma propriedade do cliente para adicionar uma condição.';
    ...
    $Self->{Translation}->{'You have no permission for this survey or stats detail!'}
        = 'Você não tem permissão para esta pesquisa ou detalhes de estatísticas!';
    ...
    $Self->{Translation}->{
        ...
        =
        'Define a quantidade de horas que um chamado deve estar fechado para envio de uma pesquisa (0 significa enviar imediatamente após fechamento). Nota: um envio tardio de pesquisa é feito pelo OTOBO Daemon, sob ativação da configuração  \'Daemon::SchedulerCronTaskManager::Task###SurveyRequestsSend\' .';
    ...
    $Self->{Translation}->{'Enable or disable the send condition check for the service.'}
        = 'Ativa ou desativa verificação de condições de serviços para envio.';
    ...
    $Self->{Translation}->{'If this regex matches, no customer survey will be sent.'}
        = 'Se esta regex for encontrada, nenhuma pesquisa será enviado para o cliente';
    ...
    $Self->{Translation}->{
        ...
        =
        'Resultados mais antigos que a quantidade configurada de dias serão excluídos. Nota: exclua resultados feitos pelo Daemon OTOBO, ativação prévia da configuração \'Task###SurveyRequestsDelete\'.';
    ...
    $Self->{Translation}->{'Surveys will not be sent to the configured email addresses.'}
        = 'As pesquisas não será enviadas para os endereços de e-mail configurados.';
    ...
    $Self->{Translation}->{'Trigger delete results (including vote data and requests).'}
        = 'Acionar os resultados da exclusão ( incluindo dados de voto e solicitações)';
